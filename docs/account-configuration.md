...
```json
{
  ...
  "escalation": {
    ...
    "contacts": [
      { "email": "manager@acme.com", "fullName": "Jane Manager" }
    ],
    "escalateToAllMembers": false
  },
  "followUp": {
  ...
  "access": {
    "teamAccess": [
      { "teamMemberId": 3, "canView": true, "canModify": true }
    ]
  }
}
```
...
```json
{
  "results": [
    { "accountType": "whatsapp", "accountId": 12, "success": true },
    { "accountType": "sms", "accountId": 999, "success": false, "error": "Account not found" }
  ]
}
```
...
| Param | Type | Description |
|---|---|---|
| `industryType` | string | Optional. Supported values: `retail`, `ecommerce`, `healthcare`, `finance`, `hospitality`, `education`, `real_estate`. Falls back to defaults if omitted or unknown. |
...
{
  "industryType": "retail",
  "keywords": ["refund", "return", "exchange", "delivery", "payment failed", "cancel order", "wrong item"]
}
```
...
```typescript
type AccountType = "whatsapp" | "messenger" | "instagram" | "email" | "sms" | "telegram";

type AccountStatus = "CONNECTED" | "REAUTH_REQUIRED" | "ERROR" | "DISCONNECTED";

type ConfigStatus = "NOT_STARTED" | "IN_PROGRESS" | "CONFIGURED" | "NEEDS_ATTENTION";

type EscalationTimeUnit = "SECONDS" | "MINUTES" | "HOURS" | "DAYS";
type FollowUpDelayUnit = "MINUTES" | "HOURS" | "DAYS";
type FollowUpContentType = "SALES" | "SUPPORT" | "BOOKING_REMINDER" | "COLD_LEAD_NUDGE" | "ABANDONED_CART" | "ONBOARDING_SETUP_HELP" | "CUSTOM";
...
const [timezones, contentTypes] = await Promise.all([
  ...
  fetch("/api/v1/configuration/options/follow-up-content-types").then(r => r.json()),
]);

// 2. Load accounts list
const { accounts } = await fetch("/api/v1/channel-accounts").then(r => r.json());
...
await fetch("/api/v1/channel-accounts/configuration/bulk", {
  ...
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    accounts: accounts.map(a => ({ accountType: a.accountType, accountId: a.accountId })),
