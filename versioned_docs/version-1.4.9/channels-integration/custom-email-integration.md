...
6. [Disconnecting Custom Email](#disconnecting-custom-email)
...
## Disconnecting Custom Email

### Remove a Custom Email Integration
...
> `accountId` is the `id` of the `EmailAccount` returned in the verification response (the `emailAccounts[].id` field).
...
1. Revokes and removes the associated `EmailAccount`
2. Deactivates the SES receipt rules for the domain
3. Removes the SNS subscription
4. Updates the `ConnectedChannel` status to disconnected
