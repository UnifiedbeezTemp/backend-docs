...
```json
{
  ...
  "dnsRecords": {
    ...
    "txt": [
      ...
      { "name": "@", "value": "unifiedbeez-verification=<verification_token>" },
      { "name": "_amazonses.yourdomain.com", "value": "<ses_verification_token>" }
    ],
    "cname": [
...
### TXT Records — SPF, Verification, and SES Identity

Three TXT records total, all from the `txt` array. Each entry has a `name` and `value` — add them as separate records at the name shown:
...
Type:  TXT
Name:  @  (root domain)
Value: unifiedbeez-verification=<token>
```
```
Type:  TXT
Name:  _amazonses.yourdomain.com
Value: <ses_verification_token>
```

> The `_amazonses` record is required for AWS SES to accept inbound email. Without it the domain stays `Pending` in SES and all inbound is rejected with a 550 error.
...
**AWS Route53:**
- MX: Record name = `<your-domain>` (or blank for apex), Type = `MX`, Value = `10 inbound-smtp.eu-west-1.amazonaws.com` — Route53 requires priority and hostname **together** in the Value field as `<priority> <hostname>`, not in separate fields
- TXT: Record name = `<your-domain>`, Type = `TXT`, Value = both `txt[].value` strings each on their own line, each in quotes — **no comma between them**. Example:
  ```
  "v=spf1 include:amazonses.com ~all"
  "unifiedbeez-verification=<token>"
  ```
- CNAME: Record name = `cname[].name`, Type = `CNAME`, Value = `cname[].value`
...
```json
{
  ...
  "dnsRecords": {
    ...
    "txt": [
      { "name": "@", "value": "v=spf1 include:amazonses.com ~all" },
      { "name": "@", "value": "unifiedbeez-verification=abc123xyz789" },
      { "name": "_amazonses.mycompany.com", "value": "abc123SEStoken==" }
    ],
    "cname": [
...
- [ ] SES domain verification TXT record added: name `_amazonses.yourdomain.com`, value from `txt[]` entry with that name
