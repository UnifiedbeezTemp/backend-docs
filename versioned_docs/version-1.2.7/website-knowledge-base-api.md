...
| Value            | Behaviour                                                                                                                                                                                                                                     |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ENTIRE_SITE`    | Sitemap-first crawl. **Fast path** (most sites): sitemap parsed synchronously in the API request — returns `COMPLETED` with `pages[]` populated immediately. **Fallback** (no sitemap): job enqueued to background worker, returns `PENDING`. |
...
**Fallback path (no sitemap found) — background worker handles discovery:**
