...
state.sessionState.computed.business_search_results; // For selectBusinessNameMatch
...
4. Backend updates answers[substep.id] = value (user-provided) or computed[field] = value (backend-derived)
...
interface SessionAnswers {
  planConfirmation?: boolean; // true = confirmed current plan
  businessIndustry: string | null;
  businessName: string | null;
  selectBusinessNameMatch: object | null; // full business object
  teamSize: string | null;
  channels: string[] | null; // internal channel values e.g. ["whatsapp"]
...
```typescript
// Transient computed fields (cleared after use):
computed.business_search_results  // Cleared at end of businessIdentity section
computed.draft_channel_configs    // Finalized at beezaroAssistants completion
computed.created_assistants       // Finalized at beezaroAssistants completion

// Persistent fields (kept throughout):
computed.firstName, computed.selectedPlan, answers.businessIndustry
answers.businessName, answers.selectBusinessNameMatch
answers.teamSize, answers.channels
answers.aiAssistantsForChannels, answers.noReplyConfiguration

// Updated fields (overwritten during edits):
Any field can be updated via:
- Normal flow: stateService.updateAnswers() / stateService.updateComputed()
- Edit flow: editSubstep() → updateAnswers()

// Cleared fields (on edit impact):
When edit affects dependent substeps, related fields cleared:
Example: Edit industry → clears computed.suggested_goals, computed.suggested_objectives
```

### Data Persistence Points

```typescript
// answers and computed are stored in:
conversation.sessionState (JSON column in DB)

// Updated after every:
1. Substep completion: answers[substepId] or computed[field] stored
2. Special handlers: multiple fields at once
3. Edit operations: target field + clear affected fields
4. Section completion: cleanup of transient computed data

// Accessed via:
state = parseSessionState(conversation.sessionState)
value = state.answers[substepId]        // user-provided
value = state.computed[fieldName]       // backend-derived
...
2. selectBusinessNameMatch
   Source: state.computed.business_search_results
   Pattern:
   options = [
   ...
   Trigger: Check state.computed.business_search_results exists
...
2. Backend searches via businessLookupService.searchBusiness(name)
   ↓
3. Stores results in state.computed.business_search_results
   ↓
...
│ • state.computed.business_search_results = []                              │
│                                                                             │
│ Solution:                                                                   │
...
describe("Edit Flow", () => {
  ...
  test("should clear affected substeps and data", async () => {
    ...
    expect(stateBefore.sessionState.computed.suggested_goals).toBeTruthy();
    ...
    expect(stateAfter.sessionState.computed.suggested_goals).toBeNull();
  });
...
describe("Assistant Management", () => {
  test("should add assistant and rebalance channels", async () => {
    const stateBefore = await getState();
    const initialCount =
      stateBefore.sessionState.computed.created_assistants.length;
    ...
    const stateAfter = await getState();
    expect(stateAfter.sessionState.computed.draft_channel_configs).toBeTruthy();
  });

  test("should delete assistant and reassign channels", async () => {
    const state = await getState();
    const assistantToDelete = state.sessionState.computed.created_assistants[1];
  ...
  test("should prevent deleting last assistant", async () => {
    // Delete all but one
    const state = await getState();
    const assistants = state.sessionState.computed.created_assistants;
...
test("should handle team size exceeding plan limit", async () => {
  ...
  // Verify plan changed
  const state = await GET("/copilot/state");
  expect(state.sessionState.computed.selectedPlan).toBe("ORGANISATION");
  expect(state.sessionState.answers.teamSize).toBe("21");
});
```
