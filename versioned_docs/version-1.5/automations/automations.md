...
interface LogicStep {
  ...
  order: number; // 0 for trigger step; 1-based for all other steps (stepOrder + 1)
...
| `"auto"`      | Predefined template steps (`SLG_*`, `SE_*`, `RN_*`) **and** branching steps (`BRANCH_SELECTOR`, `BRANCH_SUMMARY`) |
...
### Branching steps (`type: "auto"` — frontend-managed)

Auto-inserted by the frontend when a `SMART_RULE` step is configured. The backend accepts, stores, and executes them.

| `stepKey`         | DB enum           | `type` | Purpose                                                |
| ----------------- | ----------------- | ------ | ------------------------------------------------------ |
| `branch-selector` | `BRANCH_SELECTOR` | `auto` | Routes to yes/no branch based on the SMART_RULE result |
| `branch-summary`  | `BRANCH_SUMMARY`  | `auto` | Terminal display node at the end of each branch        |
...
| `branch-selector`       | `branchConfig`                         | Both `branchConfig.yes` and `branchConfig.no` non-null |
| `branch-summary`        | `branchKey`                            | Non-empty string                                       |
...
```
trigger[order=0]   → nextStepIds: ["<step[order=1].id>"]
step[order=1]      → nextStepIds: ["<step[order=2].id>"]
step[order=2]      → nextStepIds: ["<step[order=3].id>"]
...
### `BRANCH_SELECTOR` routing

`BRANCH_SELECTOR` returns **two** entries in `nextStepIds`:

```
nextStepIds[0]  →  yes-branch  (BRANCH_SUMMARY where branchKey === "yes")
nextStepIds[1]  →  no-branch   (BRANCH_SUMMARY where branchKey === "no")
```

When saving a full steps array, send both IDs:

```json
{
  "stepKey": "BRANCH_SELECTOR",
  "type": "auto",
  "nextStepIds": ["<yes-branch-step-id>", "<no-branch-step-id>"],
  "config": {
    "branchConfig": { "yes": "new-automation", "no": "this-automation" }
  }
}
```

During execution: the backend looks up the most recent `SMART_RULE` step result for the current run. `conditionsMet === true` → follows `nextStepIds[0]`; `false` → follows `nextStepIds[1]`.

`BRANCH_SUMMARY` steps are terminal — `nextStepIds: []`.
...
`trueNextStepId` / `falseNextStepId` are only relevant for `REENGAGEMENT_CAMPAIGNS` automations. For other categories, branching is handled by the `BRANCH_SELECTOR` step that follows.

---

### `branch-selector`

Auto-inserted after a `SMART_RULE` step. Config is stored as-is in the step's JSON `config` field.
...
- `branchConfig.yes` / `branchConfig.no` — UI labels indicating what each branch does (e.g. `"new-automation"`, `"this-automation"`).
- `completionStatus` is `"configured"` once both `branchConfig.yes` and `branchConfig.no` are non-null.
- Send both branch targets in `nextStepIds`: `[yesStepId, noStepId]`.

---

### `branch-summary`

Terminal display node. Config is stored as-is.

```json
{
  "branchKey": "yes",
  "selection": "new-automation"
}
```

- `branchKey` — `"yes"` or `"no"`.
- `selection` — same label from `branchConfig` (e.g. `"new-automation"`, `"this-automation"`).
- `nextStepIds: []` — always terminal.
...
| `order`                                               | `stepOrder`             | Trigger = 0; regular steps = 1-based. Backend subtracts 1 when saving regular steps sent in a batch that includes a trigger. |
...
```
GET /automations
GET /automations?category=SALES_LEAD_GENERATION&page=1&limit=20
```
...
**Response `200`** — paginated wrapper around **lightweight list items** (not the full `{ logic, layout }` contract). Use `GET /automations/:id` to fetch full step detail.
...
interface AutomationListResponse {
  ...
  limit: number; // page size used (capped at 100)
...
interface AutomationListItem {
  ...
  triggerCount: number; // number of triggers configured
  stepCount: number; // action/condition steps only (trigger excluded)
  trigger: {
    // first trigger summary, or null
    type: string; // AutomationTriggerType enum value
    name: string;
  } | null;
  lastExecution: {
    // most recent execution, or null
    status: string;
...
> `logic` and `layout` are **not** included in list responses.
...
> **Type safety:** All numeric fields inside `config` (e.g. `listIds`, `emailTemplateId`, `emailConfigId`, `whatsappAccountId`, `aiAssistantId`, `waitMinutes`, `waitDays`, etc.) **must be sent as JSON integers**, not strings. Sending `"2"` instead of `2` returns `400 Bad Request`. This is enforced server-side and is not coerced.
...
**Clearing all steps:** To wipe every step and trigger from an automation (e.g. starting over), include `"clearAllSteps": true` in the body. This must be explicit — sending `"steps": []` without `clearAllSteps: true` is a no-op (existing steps are not deleted).

```json
{ "clearAllSteps": true, "steps": [] }
```
...
New accounts start in **Beehive mode** (`User.isBeehive = true`). While in Beehive mode:

- **Automations do not execute.** Triggers are suppressed at the trigger service level — no matching automation will ever start an execution, regardless of its status.
- **Scheduled jobs** (time-based triggers, activity-threshold crons) skip Beehive accounts entirely.
- **`POST /automations/:id/test`** returns `400 Bad Request` — test executions are also blocked.
- `DRAFT`/`ACTIVE`/`PAUSED` status transitions work normally; the automation status is not affected by Beehive mode.

**Going live:** Call `POST /beehive/go-live`. This atomically sets `isBeehive = false` and records `wentLiveAt`. From that moment, any automation in `ACTIVE` status will begin firing immediately for matching contacts.

> Concurrent go-live requests are race-condition safe — only one call wins; subsequent calls get `400 Account is already live`.

---

## REENGAGEMENT_CAMPAIGNS — Per-Step Contact Eligibility

For `REENGAGEMENT_CAMPAIGNS` automations, contact eligibility is re-evaluated at **every action step**, not only at the start of the execution. Before executing each step the engine re-checks whether the contact is still an active `SUBSCRIBED` member of the automation's `campaignListId`.

If the contact has since been removed or unsubscribed from the campaign list, the execution is halted at that step (remaining steps are skipped).

This means that contacts who unsubscribe mid-flow are stopped cleanly, even if they passed the initial filter when the trigger first fired.
...
interface LogicStep {
  ...
  label: string;
  order: number; // 0 for trigger steps; 1-based for all other steps (first regular step = 1)
