...
// 1. Updated industry in answers.businessIndustry
...
// Backend actions:
// 1. Updated answers.addAutomations
...
// 2. Updated computed.selectedPlan
...
// Example state during cross-section edit:
{
  ...
  answers: { /* user inputs keyed by substepId */ },
  computed: { /* backend-derived data */ },
  editContext: {
    active: true,
    returnTo: {
      section: "channels",
...
if (editImpact?.clearedFields?.length) {
  ...
  // Implementation:
  // Set each field to null in answers or computed depending on the key
  for (const field of clearedFields) {
    if (field in state.answers) state.answers[field] = null
    else if (field in state.computed) state.computed[field] = null
  }
}
...
Step 1: Update answers.businessName
answers.businessName = "New Business Inc"

Step 2: Clear affected computed fields
computed.business_search_results = null
answers.selectBusinessNameMatch = null
computed.suggested_goals = null
computed.suggested_objectives = null
computed.ai_summary = null
...
computed.business_search_results = matches
...
// Implementation:
for (const field of clearedFields) {
  // Fields may live under answers or computed depending on the key
  if (field in state.answers) state.answers[field] = null;
  else if (field in state.computed) state.computed[field] = null;
}
...
5. Update state:
answers.channels = ["WhatsApp", "Gmail"]
...
5. Update state:
computed.created_assistants = [
  { id: 10, name: "Beezaro Alpha" },
  { id: 12, name: "Beezaro Zen" }
]
computed.draft_channel_configs = [
  { connectedChannelId: 1, aiAssistantId: 10, ... },
  { connectedChannelId: 2, aiAssistantId: 12, ... },
  { connectedChannelId: 3, aiAssistantId: 12, ... }
]
...
// computed.created_assistants stays
// computed.draft_channel_configs stays
