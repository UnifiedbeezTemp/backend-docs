...
New accounts start in **Beehive mode**. While active:

- Automations **do not execute** — triggers are suppressed and scheduled jobs skip the account entirely.
- `POST /automations/:id/test` returns `400`.
- Automation status (`DRAFT` → `ACTIVE`) can still be changed, but the automation will not run until the account goes live.

Call `POST /beehive/go-live` to exit Beehive mode. Any automation already in `ACTIVE` status begins firing immediately. See [Beehive Mode](automations.md#beehive-mode) for full details.

---

## Resetting Steps

To completely replace an automation's steps via `PUT /automations/:id`, you must explicitly pass `"clearAllSteps": true` alongside the new `steps` array. Sending `"steps": []` alone is a no-op — existing steps are preserved.
...
> **Type safety:** All numeric fields inside `config` (e.g. `listIds`, `emailTemplateId`, `aiAssistantId`, `webchatConnectedChannelId`) **must be sent as JSON integers**, not strings. Sending `"2"` instead of `2` returns `400 Bad Request`. This is enforced server-side and is not coerced.
...
> **Per-step contact eligibility:** For `REENGAGEMENT_CAMPAIGNS` automations, the engine re-checks whether the contact is still a `SUBSCRIBED` member of the automation's `campaignListId` before executing each step. If the contact has unsubscribed since the trigger fired, the execution halts and remaining steps are skipped. See [REENGAGEMENT_CAMPAIGNS — Per-Step Contact Eligibility](automations.md#reengagement_campaigns--per-step-contact-eligibility).
