...
| `accountId`        | integer   | Filter by specific account within a channel (e.g. a specific WhatsApp number ID)                 |
...
| `sortBy`           | string    | `lastMessageAt` (default) \| `createdAt` \| `unreadCount`                                        |
...
{
  "messages": [
    /* array of message objects — see Message Object below */
  ],
  "total": 120,
  "page": 1,
  "limit": 50
...
```json
{
  ...
  "metadata": {
    ...
    "senderProfilePic": "https://example.com/profile.jpg"
  },
  "tags": null,
  "rawData": null,
  "aiAssistantId": null,
  ...
  "regulatoryArchiveId": null,
  "isRegulated": false,
  "isLegalAdvice": false,
  "containsFinancialReference": false,
  "containsRegulatedProduct": null,
  "isPinned": false,
  "pinnedAt": null,
  "pinnedBy": null,
  "threadSummary": null,
  "conversation": {
    "participantId": "447700900000",
    "participantName": "John Customer",
    "participantAvatar": "https://example.com/profile.jpg",
    "status": "ACTIVE",
    "isInternal": false,
    "isGroupChat": false,
    "groupName": null,
...
> **Note on `metadata.senderProfilePic`:** Present on inbound Facebook Messenger and Instagram messages only. `null` for all other channels (WhatsApp, Telegram, SMS, Email, Webchat do not expose profile pictures via their APIs).
...
socket.on(
  "user_typing",
  ({ conversationId, userId, participantId, isTyping, timestamp }) => {
...
Two WebSocket events notify other sessions in the conversation room (peer notification only — neither writes to the database). To actually clear the unread count, call the REST endpoint separately.

```javascript
// Notify peers that specific messages were read
socket.emit("mark_messages_read", {
  conversationId: "9007199254740993",
  messageIds: ["9007199254740994", "9007199254740995"],
});

// Notify peers that ALL messages in a conversation were read (no DB read required)
...
Notifies peers that **all messages** in a conversation are read. No database query — no message IDs needed.
...
When `messageIds` is `null`, it means **all messages in the conversation were read** (sent by `mark_conversation_read`). Treat this the same as marking every message in the conversation as read.
