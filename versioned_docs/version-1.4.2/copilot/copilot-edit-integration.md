...
// Bot (SYSTEM) messages in recentMessages have this shape:
// {
//   messageType: "SYSTEM",
//   content: string,
//   metadata: {
//     section: string,
//     substep: string,           // the substep this bot message introduced
//     action: string,
//     originSubstep: string | null  // the substep the user was at when this message was triggered
//   },
//   createdAt: string
// }
//
// originSubstep is key for ?step filtering: it associates a bot transition message
// with the substep that triggered it, not the substep it introduced.
```
...
// - Update answers/computed locally
...
// 1. Updated industry in collected_data.businessIndustry
...
// 2. Set editContext.returnTo = { section: "channels", substep: "channels" }
// 3. Set editContext.active = true
...
// 2. Retrieved editContext.returnTo = { section: "channels", substep: "channels" }
// 3. Cleared edit context (editContext.active = false, editContext.returnTo = null)
...
// Backend actions:
// 1. Updated collected_data.addAutomations
...
// 4. Updated collected_data.logoUpload
...
There are two paths for changing the user's plan:

**Path A: During initial onboarding (via `POST /copilot/message`)**

When the user selects "No, Change plan" at `planConfirmation`, the flow routes to the `changePlan` substep. The user selects a new plan via `POST /copilot/message` like any other substep. The `changePlan` substep has dynamic options (filtered to exclude the current plan) — call `GET /copilot/state` first to get the populated options list.

```typescript
// POST /copilot/message when at changePlan substep
{
  inputType: "selection",
  content: "Business",
  metadata: { substepId: "changePlan", selectedOption: "BUSINESS" }
}
// → Plan switched, flow advances to businessIndustry
```

**Path B: Mid-onboarding edit (via `POST /copilot/change-plan`)**

Use this dedicated endpoint when the user wants to change their plan after `planConfirmation` is already completed (edit flow context).
...
// 2. Updated collected_data.selected_plan
...
// 6. Navigated to first affected substep OR next substep
...
interface EditContext {
  // Stored in conversation.sessionState.editContext:
  active: boolean                                        // was: is_in_edit_flow
  returnTo: {                                            // was: edit_return_position
    section: string,
    substep: string
  } | null
  affectedSubsteps: string[]                             // was: affected_substeps_in_edit
}
...
// Example state during cross-section edit:
{
  ...
  collected_data: { /* all user answers and backend-derived data in one flat map */ },
  editContext: {
    active: true,
    returnTo: {
      section: "channels",
...
State after edit:
{
  ...
  editContext: {
    active: true,                             // ✅ Edit flow active
    returnTo: {                               // ✅ Return position stored
      section: "channels",
      substep: "confirmChannelsConfiguration"
    },
    affectedSubsteps: [
...
State after completion:
{
  ...
  editContext: {
    active: true,                             // Still in edit flow
    returnTo: {                               // Still set
      section: "channels",
      substep: "confirmChannelsConfiguration"
    },
...
- editContext.active === true
- editContext.returnTo exists
...
State after return:
{
  ...
  editContext: {
    ...
    returnTo: null,          // ✅ Cleared
    affectedSubsteps: []     // ✅ Cleared
  }
}
...
Condition: editContext.active && editContext.returnTo exists
...
if (editImpact?.clearedFields?.length) {
  ...
  for (const field of clearedFields) {
    if (field in state.collected_data) state.collected_data[field] = null
  }
}
...
Step 1: Update collected_data.businessName
collected_data.businessName = "New Business Inc"

Step 2: Clear affected collected_data fields
collected_data.business_search_results = null
collected_data.selectBusinessNameMatch = null
...
editContext.active = true
editContext.returnTo = { section: "channels", substep: "channels" }
editContext.affectedSubsteps = [
  "selectBusinessNameMatch",
  "confirmBusinessData",
  "adjustGoalsObjectives"
]
...
if (state.editContext.active && state.editContext.returnTo) {
  ...
  const { section, substep } = state.editContext.returnTo
...
1. User at teamUpgradeConfirm
   ...
   → Checks edit context: editContext.active = true
...
if (state.editContext.returnTo) {
  try {
    const returnSubstep = await getSubstep(
      state.editContext.returnTo.section,
      state.editContext.returnTo.substep
    );
  } catch (error) {
    // Return position invalid - log and clear
    logger.error("Invalid return position", state.editContext.returnTo);
    await stateService.clearEditContext(conversationId);
...
if (state.editContext.active) {
  throw new BadRequestException({
    message: "Cannot edit while in edit flow",
    currentEditContext: {
      editedSubstep: state.editContext.affectedSubsteps[0],
      returnPosition: state.editContext.returnTo,
    },
    hint: "Complete current edit flow before starting a new edit",
  });
}
...
// Implementation:
for (const field of clearedFields) {
  // All fields live in collected_data
  if (field in state.collected_data) state.collected_data[field] = null;
}
...
await stateService.updateComputedData(
  conversationId,
  "business_search_results",
  null
);
...
await stateService.clearAffectedComputedData(conversationId, [
  "draft_channel_configs",
  "created_assistants",
]);
```
