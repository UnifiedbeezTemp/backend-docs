    ...
    options: [
      {
        ...
        value: {
          id: "https://acme.com",          // Places place_id OR website URL (CSE results)
          name: "Acme Electronics Inc.",
          description: "We help businesses...",  // Google snippet (CSE) or undefined (Places)
          address: "123 Main St",          // Present for Places results; absent for CSE
          phone: "+1234567890",            // Present for Places results; absent for CSE
          website: "https://acme.com",
          photo_url: "https://...",        // Identification image shown in the list
          places_photo_name: null,         // Places photo resource name (Places results only)
          source: "google_places" | "google_search",
          confidence: 0.95                 // 0.95 for Places, 0.8 for CSE
        },
        type: "radio",
...
```typescript
GET /copilot/state
GET /copilot/state?step=<sectionId>   // optional: filter recentMessages to one section

// sectionId values: businessIdentity | teamMembers | channels | fallbackLogic | aiAssistant | automation
...
Response: CopilotStateResponseDto
{
  ...
  sessionState: {
    ...
    collected_data: {
      // All user answers and backend-derived data are stored in one flat map.
      // Keys are either substepId (user answers) or internal field names (computed).
      planConfirmation: true,
      businessIndustry: "ECOMMERCE_RETAIL",
      businessName: "Acme Electronics",
      selectBusinessNameMatch: {
        name: "Acme Electronics Inc.",
        photo_url: "https://...",    // identification image (Places photo, CSE thumbnail, Clearbit, or Google favicon)
        places_photo_name: null,     // non-null only for Google Places results with a photo
        // ... other BusinessMatch fields
      },
      firstName: "John",
      ...
      // ... other fields set throughout the flow
    },
    editContext: {
      active: false,
      returnTo: null,
    ...
    phase: "onboarding"   // "onboarding" | "editing"
  },
  isActive: true,
  isCompleted: false,
  recentMessages: [
    // All messages in conversation history (full history, not capped).
    // When ?step=<sectionId> is provided, only messages where metadata.section === sectionId are returned.
    {
      messageType: "USER_SELECTION",
      content: "Yes, Proceed with my current plan",
      metadata: {
    ...
    {
      messageType: "SYSTEM",              // Bot messages use SYSTEM type (not BOT_MESSAGE)
      content: "Perfect! I'll set you up as...",
      metadata: {
        section: "businessIdentity",
        substep: "businessIndustry",      // substep this message introduced
        action: "substep_advance",
        originSubstep: "planConfirmation" // substep that triggered this bot message
      },
...
The `?step=<sectionId>` parameter is intended for rendering section-specific conversation panels. Because all messages within a section (bot intros, user responses, transition messages) share `metadata.section`, the filter captures the complete exchange without any gaps.

```typescript
// Get all messages for the businessIdentity section:
GET /copilot/state?step=businessIdentity

// Get all messages for the teamMembers section:
GET /copilot/state?step=teamMembers
...
Every message in `recentMessages` has:

```typescript
{
  messageType: "SYSTEM" | "USER_SELECTION" | "USER_INPUT" | "USER_EDIT",
  content: string,
  metadata: {
    section: string,              // The section this message belongs to
    substep: string,              // The substep this message introduced (for SYSTEM) or was submitted at (for user)
    action: string,               // e.g. "substep_advance", "user_selection", "edit", "conversation_started"
    originSubstep?: string | null, // For SYSTEM messages: the substep that triggered this bot reply
                                   // null for initialization messages
    formField?: string,           // For USER_EDIT messages
    previousValue?: any,          // For USER_EDIT messages
    substepConfig?: any | null    // Substep config snapshot at time of recording
  },
...
Logo upload is **optional**. Uploading stores the logo and returns a preview — it does **not** advance the flow. The user must explicitly confirm or skip via `POST /copilot/message` to move forward.
...
Response: CopilotResponseDto
// ⚠️ Stays on businessNotListed — does NOT advance to confirmBusinessData.
// User must call POST /copilot/message with selectedOption "confirm" or "skip" to advance.
{
  currentSubstep: {
    id: "businessNotListed",
    // ... businessNotListed substep config with "Continue" and "Skip — no logo" options
  },
  uiDirectives: {
    logoPreview: "https://s3.../business-logos/123/uuid.png"
  }
}
```

After upload, the user picks from the options on `businessNotListed`:

```typescript
// Option A: proceed with the uploaded logo
POST /copilot/message
{
  inputType: "selection",
  metadata: {
    substepId: "businessNotListed",
    selectedOption: "confirm"
  }
}

// Option B: skip logo (proceeds without one)
POST /copilot/message
{
  inputType: "selection",
  metadata: {
...
// Both options return confirmBusinessData as the next substep
Response: CopilotResponseDto
{
  currentSubstep: {
    id: "confirmBusinessData",
...
substep.type === "custom" → inputType: "selection"
...
// Note: upload is optional and does NOT advance the flow.
// Flow advances when user calls POST /copilot/message with selectedOption "confirm" or "skip".
```
...
These substeps have runtime-generated options injected by the backend — the options array in `copilotSteps.json` is empty (`[]`) and populated at runtime:

```typescript
// Always call GET /state before rendering (options injected at runtime):
"businessIndustry"; // Options from IndustryType enum
"selectBusinessNameMatch"; // Options from business search results
"channels"; // Options from user's available channels (filtered by plan/connections)
"adjustGoalsObjectives"; // Options from active goals/objectives DB
"teamUpgradeOptions"; // Plan upgrade options, filtered to exclude user's current plan
"changePlan"; // Plan change options, filtered to exclude user's current plan

// State contains the data in:
state.sessionState.collected_data.business_search_results; // For selectBusinessNameMatch
state.currentSubstepDetails.currentSubstep.options; // For all others (already injected)
```
...
4. Backend updates collected_data[substep.id] = value (user-provided) or collected_data[field] = value (backend-derived)
...
4. Backend checks edit context:
   IF (state.editContext.active && state.editContext.returnTo):
     - Clear edit context
     - Navigate to editContext.returnTo
...
All user answers and backend-derived data are stored in a single flat `collected_data` map (there is no separate `answers`/`computed` split at runtime).

```typescript
interface SessionState {
  current_section: string; // e.g. "businessIdentity"
  current_substep: string; // e.g. "businessIndustry"
  completed_substeps: string[]; // substep IDs that are done
  collected_data: {
    // User-provided answers (keyed by substepId):
    planConfirmation?: boolean;
    businessIndustry?: string | null;
    businessName?: string | null;
    selectBusinessNameMatch?: object | null; // full business object
    teamSize?: string | null;
    channels?: string[] | null; // internal values e.g. ["whatsapp"]
    confirmChannelsConfiguration?: boolean | null;
    noReplyConfiguration?: "wait" | "escalate" | "followUp" | null;
    aiAssistantsForChannels?: "single" | "multiple" | null;
    addAutomations?: string[] | false | null;
    ...
    business_search_results?: BusinessMatch[] | null;
    // BusinessMatch shape:
    // { id, name, description?, address?, phone?, website?,
    //   photo_url?,           ← identification image shown in search list (Places photo /
    //                           CSE thumbnail / Clearbit / Google favicon fallback chain)
    //   places_photo_name?,   ← Places Photos API resource name; null for CSE results
    //   source: "google_places" | "google_search", confidence: number }
    // Places results always appear first (confidence 0.95); CSE results follow (confidence 0.8).
    // CSE results are appended when Places returns ≤ 2 matches.
    business_logo?: string | null;
    ai_summary?: string | null;
    suggested_goals?: Goal[] | null;
    suggested_objectives?: Objective[] | null;
    draft_channel_configs?: ChannelConfig[];
    created_assistants?: Array<{ id: number; name: string }> | null;
    onboarding_website_id?: number | null;
    custom_industry_text?: string | null;
    goalsAdjustmentMode?: string | null;
    adjustedGoalsSummary?: string | null;
    // ... and other fields populated as the flow progresses
  };
  editContext: {
    active: boolean;
    returnTo: { section: string; substep: string } | null;
    affectedSubsteps: string[];
  };
  phase: "onboarding" | "editing";
}
```
...
collected_data.business_search_results  // Cleared at end of businessIdentity section
collected_data.draft_channel_configs    // Finalized at beezaroAssistants completion
collected_data.created_assistants       // Finalized at beezaroAssistants completion

// Persistent fields (kept throughout):
collected_data.firstName, collected_data.selected_plan
collected_data.businessIndustry, collected_data.businessName
collected_data.selectBusinessNameMatch, collected_data.teamSize
collected_data.channels, collected_data.aiAssistantsForChannels
collected_data.noReplyConfiguration

// Updated fields (overwritten during edits):
Any field can be updated via:
- Normal flow: stateService.updateCollectedData()
- Edit flow: editSubstep() → stateService.updateCollectedData()

// Cleared fields (on edit impact):
When edit affects dependent substeps, related collected_data fields are cleared to null.
Example: Edit industry → clears collected_data.suggested_goals, collected_data.suggested_objectives
```
...
// Updated after every:
1. Substep completion: collected_data[substepId] or collected_data[fieldName] stored
...
value = state.collected_data[substepId]        // user-provided answers
value = state.collected_data[fieldName]        // backend-derived data
...
5b. User selects "not listed" → businessNotListed
    ...
    - Logo upload is optional: upload stores preview, flow stays on businessNotListed
    - User selects "Continue" (confirm) or "Skip — no logo" via POST /copilot/message
    - On confirm: selected_business set with logo_url from upload (or null if not uploaded)
    - On skip: any partial upload cleared, selected_business set with logo_url: null
    - Either path triggers LLM goal generation → advances to confirmBusinessData
...
3. Backend creates ConnectedChannel records
   ...
   ↓
4. Stores in answers:
   - channels: ["whatsapp", "gmail"] (internal channel values)
   ↓
...
There are **two paths** for changing the user's plan, depending on context:

#### Path A: During initial onboarding (via `POST /copilot/message`)

When the user selects "No, Change plan" at `planConfirmation`, the flow routes to the `changePlan` substep and the user's selection is handled by `POST /copilot/message` like any other substep.

```typescript
// Step 1: planConfirmation — user selects "No, Change plan"
POST /copilot/message
{
  inputType: "selection",
  content: "No, Change plan",
  metadata: { substepId: "planConfirmation", selectedOption: false }
}
// → currentSubstep.id = "changePlan" (dynamic options, fetch via GET /state first)

// Step 2: changePlan — user selects their desired plan
POST /copilot/message
{
  inputType: "selection",
  content: "Business",
  metadata: { substepId: "changePlan", selectedOption: "BUSINESS" }
}
// → Plan is switched, flow advances to businessIndustry
// → currentSubstep.id = "businessIndustry"
```

#### Path B: Edit flow — changing plan after onboarding started (via `POST /copilot/change-plan`)

This is the dedicated endpoint used when the user wants to change their plan mid-onboarding via the edit flow. See `copilot-edit-integration.md` for details.

```typescript
POST / copilot / change - plan;
{
  newPlan: "ORGANISATION";
}
```

**Key distinction:** During the initial onboarding flow (before `planConfirmation` is completed), use `POST /copilot/message`. After `planConfirmation` is completed and you're in edit mode, use `POST /copilot/change-plan`.
...
- ✅ answers/computed/editContext field reference and lifecycle
...
├──────────────────────────┼──────────────────────────┼────────────────────────┤
│ Logo upload              │ ✅ Stores logo, returns  │ Show preview + options │
│ (businessNotListed)      │ preview on businessNot   │ "Continue" / "Skip"    │
│                          │ Listed — does NOT advance│ Must call sendMessage  │
├──────────────────────────┼──────────────────────────┼────────────────────────┤
...
│ (nested edit)            │ "Cannot edit while in    │ when editContext.active │
│                          │ edit flow"               │                        │
├──────────────────────────┼──────────────────────────┼────────────────────────┤
...
function EditButton({ substep, currentSection, value }) {
  ...
  const canEdit =
    substep.isComplete && substep.canEdit !== false && !editContext?.active;
  ...
  async function handleEdit(newValue: any) {
    ...
    try {
      ...
      if (response.uiDirectives?.editContext?.active) {
...
│ • state.editContext.active = true                                          │
│                                                                             │
│ Solution:                                                                   │
│ 1. Check state.sessionState.editContext.active before showing edit button  │
│ 2. Disable all edit buttons when in edit flow                              │
│ 3. Show "Complete current edit first" message                              │
│                                                                             │
│ UI Pattern:                                                                 │
│ {!state.sessionState.editContext.active && (                               │
...
│ • state.editContext.returnTo seems incorrect                               │
│                                                                             │
│ Debug Steps:                                                                │
│ 1. Check state.editContext.returnTo in GET /copilot/state                  │
...
describe("Edit Flow", () => {
  test("should handle same-section inline edit", async () => {
    ...
    expect(response.uiDirectives.editContext.active).toBe(false);
  ...
  test("should handle cross-section edit with navigation", async () => {
    ...
    expect(response.uiDirectives.editContext.active).toBe(true);
...
describe("File Upload", () => {
  test("should upload valid logo file and stay on businessNotListed", async () => {
    ...
    // Upload does NOT advance the flow — user must confirm or skip via POST /copilot/message
    expect(response.currentSubstep.id).toBe("businessNotListed");
    expect(response.uiDirectives.logoPreview).toMatch(/s3\.amazonaws\.com/);
  });

  test("should advance to confirmBusinessData after confirming logo", async () => {
    const response = await sendMessage({
      inputType: "selection",
      metadata: { substepId: "businessNotListed", selectedOption: "confirm" },
    });

    expect(response.currentSubstep.id).toBe("confirmBusinessData");
  });

  test("should advance to confirmBusinessData when skipping logo", async () => {
    const response = await sendMessage({
      inputType: "selection",
      metadata: { substepId: "businessNotListed", selectedOption: "skip" },
    });
...
describe("State Synchronization", () => {
  ...
  test("should persist state across GET requests", async () => {
    ...
    expect(state1.sessionState.answers).toEqual(state2.sessionState.answers);
    expect(state1.sessionState.computed).toEqual(state2.sessionState.computed);
  });
...
test("should handle cross-section edit and return", async () => {
  ...
  expect(edit.uiDirectives.editContext.active).toBe(true);
  ...
  expect(stateAfterReturn.sessionState.editContext.active).toBe(false);
  expect(stateAfterReturn.sessionState.editContext.returnTo).toBeNull();
});
...
test("should handle team size exceeding plan limit", async () => {
  ...
  expect(state.sessionState.collected_data.teamSize).toBe("21");
});
```
